...
export const updateUserData = async (req, res) => {
    try {
        ...
        //If the username is updated in the request, check the new username is available
        if(tempUser.username !== username){
            const user = await User.findOne({username})
            if(user){
                //not changing the username if it is already taken
        ...
        if(profile){  // check whether there is a profile image
            const buffer = fs.readFileSync(profile.path) // convert image in the buffer
            // upload the image
...
export const sendConnectionRequest = async (req, res) => {
    try {
        ...
        }else if (connection && connected.status == 'accepted') {
